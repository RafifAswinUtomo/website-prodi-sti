<x-layouts.admin>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Berita (Beranda)</h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                @if ($berita->gambar)
                    <div class="mb-4">
                        <img loading="lazy" src="{{ asset('storage/' . $berita->gambar) }}" class="h-24 rounded">
                    </div>
                @endif

                <form action="{{ route('admin.berita-prodi.update', $berita) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
    ...
    <select name="kategori" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
        <option value="berita" {{ old('kategori', $berita->kategori) == 'berita' ? 'selected' : '' }}>Berita (poster/flyer boleh)</option>
        <option value="kegiatan" {{ old('kategori', $berita->kategori) == 'kegiatan' ? 'selected' : '' }}>Kegiatan / Event (wajib foto asli)</option>
        <option value="prestasi" {{ old('kategori', $berita->kategori) == 'prestasi' ? 'selected' : '' }}>Prestasi (wajib foto asli)</option>
        <option value="kerjasama" {{ old('kategori', $berita->kategori) == 'kerjasama' ? 'selected' : '' }}>Kerja Sama</option>
    </select>
    @error('kategori') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="tanggal" value="{{ old('tanggal', optional($berita->tanggal)->format('Y-m-d')) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="ringkasan" rows="2"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('ringkasan', $berita->ringkasan) }}</textarea>
                    </div>

                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700">Konten Lengkap (tampil di modal)</label>
                        <textarea name="konten" rows="5"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('konten', $berita->konten) }}</textarea>
                    </div>

               <div class="mb-6">
    <label class="block text-sm font-medium text-gray-700">Urutan</label>
    <input type="number" name="urutan" value="{{ old('urutan', $berita->urutan) }}"
           class="mt-1 block w-32 border-gray-300 rounded-md shadow-sm">
</div>

<div class="mb-6">
    <label class="flex items-center gap-2">
        <input type="checkbox" name="tampil_beranda" value="1" {{ old('tampil_beranda', $berita->tampil_beranda) ? 'checked' : '' }}
