<x-layouts.admin>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Program Kelas</h2>
    </x-slot>

    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                <form action="{{ route('admin.class-programs.store') }}" method="POST">
                    @csrf
 ...
 <div class="mb-4">
    ...
    <select name="jenis_kelas" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
        ...
        <option value="reguler" {{ old('jenis_kelas') === 'reguler' ? 'selected' : '' }}>Kelas Reguler</option>
        <option value="karyawan" {{ old('jenis_kelas') === 'karyawan' ? 'selected' : '' }}>Kelas Karyawan</option>
        <option value="transfer" {{ old('jenis_kelas') === 'transfer' ? 'selected' : '' }}>Kelas Transfer</option>
    </select>
</div>
