<x-layouts.admin title="Edit Data">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Edit Informasi Beasiswa</h2>
    </x-slot>
    <div class="bg-white border rounded-lg p-6 max-w-2xl">
        <form action="{{ route('admin.informasi-beasiswa.update', $item) }}" method="POST" enctype="multipart/form-data">
            @csrf @method('PUT')
            <div class="mb-4">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.informasi-beasiswa.index') }}" class="px-4 py-2 rounded border">Batal</a>
            </div>
        </form>
    </div>
</x-layouts.admin>
