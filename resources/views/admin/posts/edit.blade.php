<x-layouts.admin>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Data</h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                @if ($post->gambar)
                    <div class="mb-4">
                        <img loading="lazy" src="{{ asset('storage/' . $post->gambar) }}" class="h-24 w-32 object-cover rounded">
                    </div>
                @endif
                        ...
                        <label class="block text-sm font-medium text-gray-700">Jenis Konten</label>
                         <select name="kategori" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach (['pengumuman' => 'Pengumuman', 'prestasi' => 'Prestasi', 'kerjasama' => 'Kerjasama', 'kegiatan' => 'Kegiatan Kemahasiswaan'] as $key => $label)
                                <option value="{{ $key }}" {{ old('type', $post->type) === $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
        ...
        <option value="informasi-beasiswa" {{ old('kategori') === 'informasi-beasiswa' ? 'selected' : '' }}>Informasi Beasiswa</option>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="isi" rows="6"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('isi', $post->isi) }}</textarea>
                    </div>
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700">Ganti Lampiran (kosongkan jika tidak diubah)</label>
                        @if ($post->lampiran)
                            <p class="text-sm text-gray-500 mb-1">
                                File saat ini: <a href="{{ asset('storage/' . $post->lampiran) }}" target="_blank" class="text-blue-700 hover:underline">lihat file</a>
                            </p>
                        @endif
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('admin.posts.index', ['type' => $post->type]) }}" class="px-4 py-2 rounded border">Batal</a>
                    </div>
                </form>
            </div>
        </div>
