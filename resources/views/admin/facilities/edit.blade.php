<x-layouts.admin>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Fasilitas</h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                @if ($facility->foto)
                    <div class="mb-4">
                        <img loading="lazy" src="{{ asset('storage/' . $facility->foto) }}" class="h-24 w-32 object-cover rounded">
                    </div>
                @endif
...
<div class="mb-4">
    ...
    <select name="kategori" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
        ...
        <option value="laboratorium" {{ old('kategori', $facility->kategori) === 'laboratorium' ? 'selected' : '' }}>Laboratorium</option>
        <option value="lembaga-sertifikasi-profesi" {{ old('kategori', $facility->kategori) === 'lembaga-sertifikasi-profesi' ? 'selected' : '' }}>Lembaga Sertifikasi Profesi</option>
    </select>
</div>
                    <div class="mb-4">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="deskripsi" rows="5"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('deskripsi', $facility->deskripsi) }}</textarea>
                    </div>
<div class="mb-6">
    ...
    <textarea name="perlengkapan" rows="5"
              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('perlengkapan', $facility->perlengkapan) }}</textarea>
