<x-layouts.admin>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Tahun {{ $milestone->tahun }}</h2>
    </x-slot>

    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                <form action="{{ route('admin.sejarah-milestones.update', $milestone) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Tahun</label>
                        <input type="number" name="tahun" value="{{ old('tahun', $milestone->tahun) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="judul" value="{{ old('judul', $milestone->judul) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="badge" value="{{ old('badge', $milestone->badge) }}"
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="deskripsi" rows="4"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('deskripsi', $milestone->deskripsi) }}</textarea>
                    </div>

                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700">Poin Capaian Penting (satu poin per baris)</label>
                        <textarea name="poin" rows="5"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('poin', $milestone->poin) }}</textarea>
                    </div>
