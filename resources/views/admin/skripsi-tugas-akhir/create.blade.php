<x-layouts.admin title="Tambah Item">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Tambah Item Skripsi/Tugas Akhir</h2>
    </x-slot>

    <div class="bg-white border rounded-lg p-6 max-w-2xl">
        <form action="{{ route('admin.skripsi-tugas-akhir.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Judul</label>
                <input type="text" name="judul" value="{{ old('judul') }}"
                       placeholder="Contoh: Panduan Skripsi"
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.skripsi-tugas-akhir.index') }}" class="px-4 py-2 rounded border">Batal</a>
            </div>
        </form>
    </div>
</x-layouts.admin>
